...
// Tipes y mocks
import { PROVEEDORES_EJEMPLO } from "@/mocks/proveedores";
...
export default function ListadoProveedores() {
  const [proveedores, setProveedores] =
    useState<Proveedor[]>(PROVEEDORES_EJEMPLO);
  ...
  const exportarCSV = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      const fileName = `proveedores_${filtroActivo}_${
      ...
      if (success) {
        toast.success(
          `Archivo CSV generado correctamente (${proveedoresFiltrados.length} registros)`
        );
      } else {
