...
import { DISTRIBUIDORES_EJEMPLO } from "@/mocks/distribuidores";
...
export default function ListadoDistribuidores() {
  ...
  const handleCambiarEstado = (distribuidor: Distribuidor) => {
    const nuevoEstado =
      distribuidor.estado === "Activo" ? "Inactivo" : "Activo";
    setDistribuidores((prev) =>
      prev.map((p) =>
        p.id === distribuidor.id ? { ...p, estado: nuevoEstado } : p
      )
    );
    toast.success(
      `Distribuidor ${distribuidor.nombre} marcado como ${nuevoEstado}`
    );
  };
  ...
  const exportarCSV = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      const fileName = `distribuidores_${filtroActivo}_${
      ...
      if (success) {
        toast.success(
          `Archivo CSV generado correctamente (${distribuidoresFiltrados.length} registros)`
        );
      } else {
